...
import { ArrowLeftIcon, ArrowRightIcon, ChevronLeftIcon } from "./Icons";
...
const DashboardViewer: React.FC<DashboardViewerProps> = ({ item, onBack }) => {
	...
	const [viewMode, setViewMode] = useState<"simulation" | "gallery">(
		item.demoType ? "simulation" : "gallery"
	);
	...
	return (
		<div className="fixed inset-0 z-50 bg-slate-950 text-slate-200 overflow-y-auto animate-fade-in-up">
			...
			<div className="sticky top-0 z-10 bg-slate-950/90 backdrop-blur-md border-b border-slate-800 px-6 py-4 flex items-center justify-between">
				...
				<h2 className="hidden md:block text-lg font-bold text-white truncate max-w-md">
					{item.title}
				</h2>
				...
				<div className="lg:col-span-2 space-y-4">
					<div className="relative rounded-2xl overflow-hidden bg-slate-900 border border-slate-800 shadow-2xl aspect-video group">
						{viewMode === "simulation" && simulationComponent ? (
							...
							<div className="w-full h-full">{simulationComponent}</div>
						) : (
					...
					<div>
						...
						<div className="flex flex-wrap gap-2 mb-6">
							{item.techBadges.map((badge) => (
								<span
									key={badge}
									className="px-3 py-1 text-xs font-bold uppercase tracking-wider text-blue-300 bg-blue-950/30 border border-blue-900/50 rounded-md">
									{badge}
								</span>
							))}
						</div>
						...
						{viewMode === "simulation" ? (
							<div className="text-sm text-slate-300 space-y-3">
								...
								<p className="text-slate-400 text-xs">
									This interactive component demonstrates the logic and state
									management used in the actual production system.
								</p>
							</div>
						) : (
