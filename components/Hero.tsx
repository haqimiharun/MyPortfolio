    ...
    <section id="home" className="min-h-screen flex flex-col justify-center items-center relative pt-20 overflow-hidden">
      ...
      <div className="z-10 max-w-5xl mx-auto px-6 flex flex-col items-center text-center space-y-8">
        ...
        <div className="relative w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-slate-800/80 shadow-2xl animate-fade-in-up mb-4 overflow-hidden group">
          ...
          <img 
            src={`https://ui-avatars.com/api/?name=${encodeURIComponent(profile.name)}&background=10b981&color=fff&size=256`} 
            alt={profile.name} 
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 animate-fade-in-up delay-500 pt-6">
          ...
          <div className="flex gap-3">
            <a href={`mailto:${profile.email}`} className="p-3 rounded-full bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white transition-all border border-slate-700 hover:border-emerald-500/50 hover:-translate-y-1" aria-label="Email">
              <MailIcon className="w-5 h-5" />
            </a>
            <a href={`https://${profile.linkedin}`} target="_blank" rel="noopener noreferrer" className="p-3 rounded-full bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white transition-all border border-slate-700 hover:border-emerald-500/50 hover:-translate-y-1" aria-label="LinkedIn">
              <LinkedinIcon className="w-5 h-5" />
            </a>
            {profile.github && (
              <a href={`https://${profile.github}`} target="_blank" rel="noopener noreferrer" className="p-3 rounded-full bg-slate-800/80 hover:bg-slate-700 text-slate-300 hover:text-white transition-all border border-slate-700 hover:border-emerald-500/50 hover:-translate-y-1" aria-label="GitHub">
      ...
      <div className="absolute bottom-10 left-1/2 -translate-x-1/2 animate-bounce text-slate-500 hover:text-emerald-400 transition-colors cursor-pointer" onClick={() => document.getElementById('resume')?.scrollIntoView({behavior: 'smooth'})}>
