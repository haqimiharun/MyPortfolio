		...
		const interval = setInterval(() => {
			setDataPoints((prev) => {
				const next = [
					...
					Math.max(
						10,
						Math.min(90, prev[prev.length - 1] + (Math.random() * 20 - 10))
					),
				];
				return next;
			});
			setRequestRate((prev) => {
				const next = [
					...
					Math.max(
						50,
						Math.min(300, prev[prev.length - 1] + (Math.random() * 40 - 20))
					),
				];
				return next;
			});
	...
	const generatePath = (data: number[], color: string) => {
		...
		return (
			<svg
				...
				<path
					d={`M0,100 L0,${100 - (data[0] / max) * 100} ${points
						.map((p) => `L${p}`)
						.join(" ")} L100,100 Z`}
					fill={`url(#grad-${color})`}
				/>
				<path
					d={`M0,${100 - (data[0] / max) * 100} ${points
						.map((p) => `L${p}`)
						.join(" ")}`}
					fill="none"
					stroke={color}
			...
			<div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full">
				...
				<div className="bg-slate-950/50 border border-slate-700/50 rounded-lg p-4 flex flex-col">
					...
					<div className="flex-1 relative">
						{generatePath(dataPoints, "#10b981")}
					</div>
				...
				<div className="bg-slate-950/50 border border-slate-700/50 rounded-lg p-4 flex flex-col">
					...
					<div className="flex-1 relative">
						{generatePath(requestRate, "#3b82f6")}
					</div>
					<div className="mt-2 text-right text-2xl font-mono text-white">
...
const PipelineSimulation: React.FC = () => {
	...
	const getStatusColor = (status: string) => {
		switch (status) {
			...
			case "running":
				return "bg-blue-900/50 border-blue-500 text-blue-200 animate-pulse";
			case "success":
			...
			<div className="flex flex-wrap justify-center items-center gap-4 md:gap-8 mt-6">
				{stages.map((stage, idx) => (
					<div key={stage.id} className="flex items-center">
						<div
							className={`w-24 h-24 md:w-32 md:h-32 rounded-full flex flex-col items-center justify-center border-2 transition-all duration-500 ${getStatusColor(
								stage.status
							)}`}>
							<span className="text-xs md:text-sm font-bold">{stage.name}</span>
							{stage.status === "success" && (
								<BadgeCheckIcon className="w-5 h-5 md:w-6 md:h-6 mt-1" />
							)}
							{stage.status === "running" && (
								<div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin mt-2"></div>
							)}
						</div>
						{idx < stages.length - 1 && (
							<div
								className={`w-8 md:w-16 h-1 transition-colors duration-500 ${
									stage.status === "success" ? "bg-emerald-500" : "bg-slate-700"
								}`}></div>
						)}
					</div>
				))}
		...
		<div className="bg-slate-950 w-full h-full p-6 relative rounded-xl border border-slate-800 flex items-center justify-center overflow-hidden">
			...
			<div className="relative z-10 flex flex-col gap-12 items-center">
				...
				<div className="flex flex-col items-center gap-2 group cursor-pointer">
					...
					<div className="text-xs text-slate-500 group-hover:text-emerald-500 transition-colors">
						Port 80/443
					</div>
				</div>
					...
					<div className="flex flex-col md:flex-row gap-8">
						...
						<div className="flex flex-col items-center gap-2">
							<div className="w-32 h-32 rounded-xl bg-indigo-900/20 border border-indigo-500/50 flex flex-col items-center justify-center gap-2 hover:bg-indigo-900/40 transition-colors">
								...
								<span className="text-[10px] bg-indigo-950 px-2 py-1 rounded text-indigo-400">
									fpm-alpine
								</span>
							</div>
						</div>
						...
						<div className="flex flex-col items-center gap-2">
							<div className="w-32 h-32 rounded-xl bg-blue-900/20 border border-blue-500/50 flex flex-col items-center justify-center gap-2 hover:bg-blue-900/40 transition-colors">
								...
								<span className="text-[10px] bg-blue-950 px-2 py-1 rounded text-blue-400">
									Volume Data
								</span>
							</div>
						</div>
